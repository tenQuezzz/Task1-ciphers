        ...
        for b in range(256):
            if gcd(a, 256) == 1:
                ...
                if isEnglish(txt):
                    ...
                    break
    a, b = keys
    decrypted_msg = affine.decrypt_data(encrypted_data, find_mod_inverse(a, 256), b)
        ...
        for b in range(256):
            if gcd(a, 256) == 1:
                ...
                if a_inv is None:
                    continue
                count += 1
                decrypted_data = affine.decrypt_data(encrypted_png_data[:2], a_inv, b)
                if decrypted_data == [137, 80]:
                    print(f'Keys are: a={a}, b={b}, Attempts: {count}')
                    keys = [a, b]
    a, b = keys
    decrypted_png_data = affine.decrypt_data(encrypted_png_data, find_mod_inverse(a, 256), b)
